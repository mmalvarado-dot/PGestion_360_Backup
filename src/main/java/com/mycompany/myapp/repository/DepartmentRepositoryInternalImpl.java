...
class DepartmentRepositoryInternalImpl extends SimpleR2dbcRepository<Department, Long> implements DepartmentRepositoryInternal {
    ...
    private static final Table parentDepartmentTable = Table.aliased("department", "parentDepartment");
    ...
    RowsFetchSpec<Department> createQuery(Pageable pageable, Condition whereClause) {
        ...
        columns.addAll(DepartmentSqlHelper.getColumns(parentDepartmentTable, "parentDepartment"));
        SelectFromAndJoinCondition selectFrom = Select.builder()
            .select(columns)
            .from(entityTable)
            .leftOuterJoin(parentDepartmentTable)
            .on(Column.create("parent_department_id", entityTable))
            .equals(Column.create("id", parentDepartmentTable));
    ...
    private Department process(Row row, RowMetadata metadata) {
        ...
        entity.setParentDepartment(departmentMapper.apply(row, "parentDepartment"));
        return entity;
    }
